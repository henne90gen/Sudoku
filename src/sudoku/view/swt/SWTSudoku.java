...
class SWTSudoku {
	...
	SWTSudoku(SWTView view, SudokuModel sudoku, Composite tabComposite) {
		...
		timeLabel = SWTHelper.INSTANCE.getLabel(Display.getCurrent(), tabComposite, messageLabel,
				SWTConstants.TIME_LABEL_TEXT);

		operationsLabel = SWTHelper.INSTANCE.getLabel(Display.getCurrent(), tabComposite, timeLabel,
				SWTConstants.OPERATIONS_LABEL_TEXT);

		Listener solverSelectorListener = event -> reset();
		solverSelector = SWTHelper.INSTANCE.getSolverSelector(tabComposite, operationsLabel, solverSelectorListener);

		// create grid of text components
		grid = new Text[81];
		for (int row = 0; row < 9; row++) {
			for (int col = 0; col < 9; col++) {
				Text cell = SWTHelper.INSTANCE.getCell(sudoku, Display.getCurrent(), tabComposite, grid, row, col);
				if (row == 0) {
	...
	private void addListeners(Button solveBtn, Button resetBtn, Button printBtn) {
		solveBtn.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				view.getController()
						.startSolver(sudoku, getSelectedSolver());
			}
	...
	private void handleFinishedSolvingEvent(SudokuEvent event) {
		...
		for (int row = 0; row < 9; row++) {
			for (int col = 0; col < 9; col++) {
				int currentNumber = sudoku.getNumber(row, col);
				if (currentNumber != 0) {
	...
	private void handleSetNumberEvent(SudokuEvent event) {
		resetHighlightedCell();
		highlightedRow = event.getPosition()
				.getRow();
		highlightedCol = event.getPosition()
				.getCol();
	...
	private void resetHighlightedCell() {
		Display.getCurrent()
				.syncExec(() -> grid[highlightedRow * 9 + highlightedCol]
						.setBackground(new Color(Display.getCurrent(), 0, 0, 0)));
	}

	public void registerListener(ISudokuController controller) {
		SudokuListener listener = event -> {
			view.getDisplay()
					.syncExec(() -> {
		...
		controller.addListener(sudoku, listener);
	}
}
